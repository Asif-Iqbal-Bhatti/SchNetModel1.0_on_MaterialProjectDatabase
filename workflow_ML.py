...
print('2M_ARG:: ', len(argo_Dir))

#============= EVOLUTION OF WORKFLOW
dLDATA = ['A', 'B', 'C']

outFile2.write(
	f"WorkflowStep,Dir,Subdir,IonicStep,TotalEnergy/atom,CorrTotalEnergy/atom,ehull\n"
)
		...
		try:
			...
			for dl in dLDATA:
				...
				for ionstep, at in enumerate(read_xmlF, start=1):	
					nn.append(f"{ionstep}")
					ee.append(f"{at.get_total_energy() / at.get_global_number_of_atoms()}")
			...
			for dl in reversed(dLDATA):
				...
				for u, at in enumerate(read_xmlF, start = 1):
					if oo <=  key_list[0]:
						...
						dif1 = k2+vvdiff[u-1]
						outFile2.write(
							f"{wf1}, {p}, {dl}, {kk[u - 1]}, {vv[u - 1]}, {vv[u - 1]}, {k2 + vvdiff[u - 1]}\n"
						)
					...
					if oo > key_list[0] and oo <= key_list[1]:
						...
						outFile2.write(
							f"{wf1}, {p}, {dl}, {kk[u - 1]}, {vv[u - 1]}, {vv[u - 1] + val_list[0][0]}, {dif1 + dif2[u - 1]}\n"
						)
				...
				for u, at in enumerate(read_xmlF, start = 1):
					if oo > key_list[1]:	
						...
						outFile2.write(
							f"{wf1}, {p}, {dl}, {kk[u - 1]}, {vv[u - 1]}, {vv[u - 1] - dife[0]}, {dif3 + dif4[u - 1]}\n"
						)		
			...
			pass
print(f"Total_Entries_in_db:: {data_cnt}")
print(f"Total_workflow_in_db:: {wf1 - 1}")
